...
{
    // variables
    public $ssbl_checkboxes;

    // opening form tag
    public function open($wrap, $action = '', $class = '')
    {
        $return = '<div class="'.($wrap ? 'ssbl-form-wrap' : null).'">';
        $return .= '<form class="form-horizontal '.$class.'" id="ssbl-admin-form" method="post" action="'.$action.'">';

        // required hidden fields
        $return .= wp_nonce_field('ssbl_save_settings', 'ssbl_save_nonce');
        $return .= '<input type="hidden" name="ssbl_options" />';
    ...
    {
        // save button
        $return = '<button type="submit" id="submit" class="ssbl-btn-save btn btn-lg btn-primary"><i class="fa fa-floppy-o"></i></button>';

        // success button
        $return .= '<button type="button" class="ssbl-btn-save-success btn btn-lg btn-success"><i class="fa fa-check"></i></button>';
    ...
    // inline checkboxes
    public function ssbl_checkboxes($opts)
    {
        ...
        $input .= '<div class="">';
            ...
            // add all checkboxes
            array_walk($opts['checkboxes'], array($this, '_ssbl_add_checkboxes'));
        $input .= $this->ssbl_checkboxes;
    ...
    // checkboxes
    private function _ssbl_add_checkboxes($value, $key)
    {
        $this->ssbl_checkboxes .= '<label class="checkbox-inline no_indent">
									'.$key.'<br />
    ...
    public function ssbl_input($opts)
    {
        // check if opts passed is an array
        if (!is_array($opts)) {
            return 'Variable passed not an array';
        ...
        switch ($opts['type']) {
            ...
            case 'image_upload':
                $input .= '<div class="input-group">
						    ...
						    <span class="input-group-btn">
						      <button id="upload_'.$opts['name'].'_button" class="ssblUpload ssbl_upload_btn btn btn-default" data-ssbl-input="'.$opts['name'].'" type="button">Upload</button>
						    </span>
						  </div>';
            break;
            ...
            case 'colorpicker':
                ...
                $input .= '<input id="'.$opts['name'].'" name="'.$opts['name'].'" type="text" class="ssbl-colorpicker form-control" value="'.$opts['value'].'" placeholder="#4582ec" style="border-color: '.($opts['value'] != '' ? $opts['value'] : '#eaeaea').'" />';
            break;
