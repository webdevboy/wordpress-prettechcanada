...
class DUP_PRO_Build_Progress
{
    ...
    public function set_build_mode()
    {
        ...
        if ($this->current_build_mode == -1) {
            ...
            } else if($global->archive_build_mode == DUP_PRO_Archive_Build_Mode::ZipArchive) {
            ...
            {
                DUP_PRO_LOG::trace("DupArchive is global setting so setting this package to ziparchive");
...
class DUP_PRO_Package
{
    ...
    function __construct()
    {
        ...
        $this->Database  = new DUP_PRO_Database();
    ...
    public static function get_from_json($json_string)
    {
        ...
        $package->Database = new DUP_PRO_Database();
    ...
    {
        if($this->ZipMode == DUP_PRO_Archive_Build_Mode::DupArchive)
        {
            $extension = 'daf';
        }
        else
    ...
    public function save()
    {
        ...
        if ($this->ID == -1 || empty($this->ID)) {
            DUP_PRO_LOG::profileEvent('Package core save', true);
            /* @var $global DUP_PRO_Global_Entity */
            $global = DUP_PRO_Global_Entity::get_instance();
            ...
            $results = $wpdb->insert($wpdb->base_prefix."duplicator_pro_packages",
                array(
                'name' => $this->Name,
                'hash' => $this->Hash,
            ...
            DUP_PRO_LOG::profileEvent('Package core save', false);
    ...
    {
        DUP_PRO_LOG::profileEvent('Package update', true);
        global $wpdb;
        $packageObj = json_encode($this);
        ...
        $wpdb->query($sql);
        DUP_PRO_LOG::profileEvent('Package update', false);
    }
