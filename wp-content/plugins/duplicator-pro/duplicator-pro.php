...
if(isset($_REQUEST['dup_pro_profiling']))
{
    $value = (((bool)$_REQUEST['dup_pro_profiling']));

    update_option(DUP_PRO_Constants::PROFILING_OPTION_NAME, $value);

    if($value)
    {
        DUP_PRO_LOG::trace("Profiling turned on");
    } else {
        DUP_PRO_LOG::trace("Profiling turned off");
    }
}

if(isset($_REQUEST['dup_pro_beta']))
{
    $value = (((bool)$_REQUEST['dup_pro_beta']));

    update_option(DUP_PRO_Constants::BETA_FEATURES_OPTION_NAME, $value);

    if($value)
    {
        DUP_PRO_LOG::trace("Beta features turned on");
    } else {
        DUP_PRO_LOG::trace("Beta features turned off");
    }
}
...
if(get_option(DUP_PRO_Constants::PROFILING_OPTION_NAME))
{
   ...
   // DUP_PRO_LOG::traceObject("Retrieving profile logs entity", $profileLogsEntity);
...
if (is_admin() == true) {
	...
	function duplicator_pro_admin_footer() 
	{
        ...
        $profiling_on = get_option(DUP_PRO_Constants::PROFILING_OPTION_NAME, false);
...
HTML;
    ...
    function duplicator_pro_menu()
    {
        ...
        $perms                             = 'export';
        $perms                             = apply_filters($wpfront_caps_translator, $perms);
        $page_packages                     = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, DUP_PRO_U::__('Packages'), DUP_PRO_U::__('Packages'), $perms,
            DUP_PRO_Constants::$PACKAGES_SUBMENU_SLUG, 'duplicator_pro_get_menu');
        $GLOBALS['DUP_PRO_Package_Screen'] = new DUP_PRO_Package_Screen($page_packages);


        $perms          = 'manage_options';
        $perms          = apply_filters($wpfront_caps_translator, $perms);
        $page_schedules = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, DUP_PRO_U::__('Schedules'), DUP_PRO_U::__('Schedules'), $perms,
            DUP_PRO_Constants::$SCHEDULES_SUBMENU_SLUG, 'duplicator_pro_get_menu');

        $perms        = 'manage_options';
        $perms        = apply_filters($wpfront_caps_translator, $perms);
        $page_storage = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, DUP_PRO_U::__('Storage'), DUP_PRO_U::__('Storage'), $perms, DUP_PRO_Constants::$STORAGE_SUBMENU_SLUG,
            'duplicator_pro_get_menu');

        $perms          = 'manage_options';
        $perms          = apply_filters($wpfront_caps_translator, $perms);
        $page_templates = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, DUP_PRO_U::__('Templates'), DUP_PRO_U::__('Templates'), $perms,
            DUP_PRO_Constants::$TEMPLATES_SUBMENU_SLUG, 'duplicator_pro_get_menu');

        $perms      = 'manage_options';
        $perms      = apply_filters($wpfront_caps_translator, $perms);
        $page_tools = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, DUP_PRO_U::__('Tools'), DUP_PRO_U::__('Tools'), $perms, DUP_PRO_Constants::$TOOLS_SUBMENU_SLUG,
            'duplicator_pro_get_menu');

        $perms         = 'manage_options';
        $perms         = apply_filters($wpfront_caps_translator, $perms);
        $page_settings = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, DUP_PRO_U::__('Settings'), DUP_PRO_U::__('Settings'), $perms, DUP_PRO_Constants::$SETTINGS_SUBMENU_SLUG,
            'duplicator_pro_get_menu');
