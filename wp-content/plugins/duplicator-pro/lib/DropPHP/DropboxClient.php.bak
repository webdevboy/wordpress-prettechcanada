            ...
            while (($end_of_file == false) && ($time_passed < $max_upload_time_in_sec) & ($error_present == false))
            {
                ...
                if (empty($response) == false)
                {
                    if (!empty($response->error))
                    {
                        ...
                        DUP_PRO_U::log("rolling back dropbox due to $error_message");
						...
						{							
							...
							{								
								...
								DUP_PRO_U::log("Forcing old offset to be $old_offset");
							}
							else
							{
								DUP_PRO_U::log_object("Something wrong with parsing matches, retval=$preg_retval", $matches);
							}
						}
						else
						{
							DUP_PRO_U::log("Response error doens't contain expected");
						}
