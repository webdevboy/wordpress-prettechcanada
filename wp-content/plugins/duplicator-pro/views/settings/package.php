...
$is_beta_enabled = get_option(DUP_PRO_Constants::BETA_FEATURES_OPTION_NAME);

if($is_beta_enabled) {
    $beta_display = '';
} else {
    $beta_display = 'display:none;';
}
...
if (isset($_POST['action']) && $_POST['action'] == 'save')
{
    ...
    if(isset($_REQUEST['ziparchive_mode']))
    {
         ...
         <tr>
            ...
            <td>
						...
						<?php DUP_PRO_U::_e("Archive Compression"); ?>
							...
							data-tooltip="<?php DUP_PRO_U::_e('Controls  archive compression. Turning compression off can improve build stability and speed but packages will be larger. Can be toggled when using ZipArchive on a PHP 7+ system or Shell Exec Zip.'); ?>"></i>
...
//								DUP_PRO_U::_e("If your host is not able or willing to enable Shell Exec zip and you wish to use it please refer to {$providers} for recommended hosts");
                ...
                <!-- DUPARCHIVE -->
                <input style="<?php echo $beta_display; ?>" onclick="DupPro.UI.SetArchiveOptionStates();" type="radio" name="archive_build_mode" id="archive_build_mode3"  value="<?php echo DUP_PRO_Archive_Build_Mode::DupArchive; ?>" <?php echo DUP_PRO_UI::echoChecked($global->archive_build_mode == DUP_PRO_Archive_Build_Mode::DupArchive); ?> />
                <label style="<?php echo $beta_display?>" for="archive_build_mode3"><?php DUP_PRO_U::_e("DupArchive"); ?></label> &nbsp;
